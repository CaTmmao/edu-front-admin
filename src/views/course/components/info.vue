  ...
  methods: {
    ...
    initCourseInfo(data) {
      ...
      this.selectedCategoryValue = [
        undefined,
        this.courseInfo.categorySecondId,
      ];
    ...
    handleCategoryChange() {
      this.selectedCategoryValue.forEach((item) => {
        if (item) {
          this.courseInfo.categorySecondId = item;
        }
      });
    },
    ...
    async upload(file) {
      ...
      await ossApi.uploadPhoto(formData).then((res) => {
    ...
    async add() {
      try {
        ...
        let { courseInfo } = this;
        this.assignFirstCategoryBySecondCategory(courseInfo);
        return courseInfo;
      } catch (e) {
        throw e;
      }
    ...
    // 根据二级分类赋值一级分类
    assignFirstCategoryBySecondCategory(courseInfo) {
      // 已经选择分类
      if (courseInfo.categorySecondId) {
        this.categoryList.forEach((item) => {
          if (item.id === courseInfo.categorySecondId) {
            courseInfo.categoryFirstId = item.parentId;
          }
          
          if (item.children) {
            item.children.forEach((child) => {
              if (child.id === courseInfo.categorySecondId) {
                courseInfo.categoryFirstId = child.parentId;
              }
            });
          }
        });
